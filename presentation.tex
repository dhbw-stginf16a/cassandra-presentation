...
\begin{frame}{History}
  \begin{itemize}
    \item 2008, Developed by Facebook for inbox searching \cite{cassandra_paper}
    \item 2010, Proper Apache project \cite{cassandra_apache}
    \item Mainly developed and supported by Datastax \cite{cassandra_oreilly}
    \item 2019, 11th most popular database overall \cite{db_engines}
  \end{itemize}
...
\begin{frame}{Properties \cite{cassandra_paper, cassandra_apache}}
  \begin{itemize}
...
\begin{frame}{Wide Column Store}
  ...
  \onslide<6->
  "Sparse, distributed multi-dimensional sorted map" \cite{bigtable}

  \note<2>[item]{Unlike Vertica or SAP HANA}
  \note<6>[item]{Quote from original paper}
\end{frame}

\begin{frame}{Wide Column Store \cite{wideColumnGraphic}}
  \begin{center}
...
\begin{frame}{Suitable Use-Cases \cite{netflix, apple, ebay}}
  ...
  \begin{itemize}
    \item<1-> Recording of transactions
      \begin{itemize}
        \item Shopping cart (fraud detection) \cite{usecaseFraud}
...
\begin{frame}{How to model data - Chebotko Diagram \cite{cassandra_oreilly, chebotkoPaper}}
  \begin{center}
    \includegraphics[width=0.75\columnwidth]{resources/model_example_chebotko_step3.png}
  \end{center}
\end{frame}

\begin{frame}{How to model data - Primary Key \cite{cassandra_oreilly, chebotkoPaper}}
  \begin{center}
