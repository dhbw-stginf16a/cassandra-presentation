...
\begin{frame}[fragile]{Secondary index - Why}
  ...
  ----------+-----
      timmy |   5
  \end{semiverbatim}
  \note[item]{Unpredicatable performance \rightarrow ALLOW FILTERING}
  \note[item]{Has to go through all nodes and all of their data}
\end{frame}
...
\begin{frame}{Secondary index - Summary}
  ...
  \end{itemize}
  \note[item]{Partition key \rightarrow only needs to search single node}
\end{frame}
...
\begin{frame}{Secondary index - When not to use}
  ...
  \end{itemize}
  \note[item]{Low chance of finding - no partition key \rightarrow queries all nodes}
  \note[item]{Results in huge index rows}  % TODO: Think about a good explanation
  \note[item]{Updates have to update two tables}
  \note[item]{Tombstones are stored in index - hard limit of 100K \rightarrow Query FAILS}
\end{frame}
...
\begin{frame}{Security}
  \begin{itemize}
    ...
    \item Use roles
  \end{itemize}
  \note[item]{Basic linux due diligence}
  \note[item]{TLS is always good \rightarrow Encryption, Authenticity}
  \note[item]{There are no users, only roles (good RBAC model)}
\end{frame}
